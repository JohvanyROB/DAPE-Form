...
%Obstacle avoidance
cBetax = 1.5; cBetay = 1.5; cBetaz = 1.5;
cdBetax = 0.5; cdBetay = 0.5; cdBetaz = 0.5;
...
%------------------------------------
    ...
    open_system('nav_flocking33')    %32 works fine
    sim('nav_flocking33.slx',tsim);
...
lw=2.2; lw2 = 2; t=tout;    
%horizontal motion
xi=X(:,1:4);  yi=X(:,5:8); zi=X(:,9:12); vxi=X(:,13:16); vyi=X(:,17:20); vzi=X(:,21:24);
N = size(X1) ;
...
figure(1)
    ...
    pos3 = plot3(xi(2:4:end,2:4), yi(2:4:end,2:4), zi(2:4:end,2:4),'LineWidth',lw); 
    set(pos3,{'LineStyle'},{'-';'-';'-'})
    grid on;    
    legend((pos3),'$x_1$','$x_2$','$x_3$', 'AutoUpdate','off','interpreter','latex','Fontsize',15);
    xlabel('$p_x$ [m]','interpreter','latex')
    ylabel('$p_y$ [m]','interpreter','latex')
    zlabel('$p_z$ [m]','interpreter','latex')
    hold on;
    line([xi(3000,2:4), xi(3000,2)], [yi(3000,2:4), yi(3000,2)], [zi(3000,2:4), zi(3000,2)], 'LineStyle', '--', 'Color', 'b', 'LineWidth',lw2);
    hold on;
    line([xi(10000,2:4), xi(10000,2)], [yi(10000,2:4), yi(10000,2)], [zi(10000,2:4), zi(10000,2)], 'LineStyle', '--', 'Color', 'b', 'LineWidth',lw2);
    hold on;
    line([xi(20000,2:4), xi(20000,2)], [yi(20000,2:4), yi(20000,2)], [zi(20000,2:4), zi(20000,2)], 'LineStyle', '--', 'Color', 'b', 'LineWidth',lw2);
    hold on
    [A,B,C] = sphere;
    r = 0.15;
    A1 = A * r;
